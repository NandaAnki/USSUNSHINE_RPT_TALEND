...
package uscomm_sunshine_rpt_qa.sftp_conn_0_1;
 ...
 * @status
 */
public class SFTP_CONN implements TalendJob {
	static {
		System.setProperty("TalendJob.log", "SFTP_CONN.log");
	}

	private static org.apache.logging.log4j.Logger log = org.apache.logging.log4j.LogManager.getLogger(SFTP_CONN.class);
	...
	private final String jobName = "SFTP_CONN";
				...
				try {
					for (java.lang.reflect.Method m : this.getClass().getEnclosingClass().getMethods()) {
						if (m.getName().compareTo(currentComponent + "_error") == 0) {
							m.invoke(SFTP_CONN.this, new Object[] { e, currentComponent, globalMap });
							break;
						}
					}
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				if (log.isDebugEnabled()) {
					class BytesLimit65535_tFTPConnection_1 {
						public void limitLog4jByte() throws Exception {
							...
							log4jParamters_tFTPConnection_1.append(" | ");
							log4jParamters_tFTPConnection_1.append("PASSPHRASE" + " = " + String.valueOf(
									"enc:routine.encryption.key.v1:AT2+nh9Bag1CNtZD+eO5IULe8T5WC+YO+QgXHvyz/BT5pA==")
									.substring(0, 4) + "...");
							log4jParamters_tFTPConnection_1.append(" | ");
				...
				class MyUserInfo implements com.jcraft.jsch.UserInfo, com.jcraft.jsch.UIKeyboardInteractive {
					...
					String decryptedPassphrase_tFTPConnection_1 = routines.system.PasswordEncryptUtil.decryptPassword(
							"enc:routine.encryption.key.v1:KIxr2ERcr1u7JaWi1GiB/4gQGWgoJ2rtjfz1/iGQlf3SHA==");
	...
	public void tFTPFileList_1Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				if (log.isDebugEnabled()) {
					class BytesLimit65535_tFTPFileList_1 {
						public void limitLog4jByte() throws Exception {
							...
							log4jParamters_tFTPFileList_1.append(" | ");
							log4jParamters_tFTPFileList_1
									.append("REMOTEDIR" + " = " + "\"/SUNSHINE_CONCUR/EXTRACT_INPUT_FILES/\"");
							log4jParamters_tFTPFileList_1.append(" | ");
							log4jParamters_tFTPFileList_1.append("DIR_FULL" + " = " + "false");
							log4jParamters_tFTPFileList_1.append(" | ");
							log4jParamters_tFTPFileList_1
									.append("FILES" + " = " + "[{FILEMASK=" + ("\"*.txt\"") + "}]");
							log4jParamters_tFTPFileList_1.append(" | ");
							if (log.isDebugEnabled())
				...
				maskList_tFTPFileList_1.add("*.txt");
				com.jcraft.jsch.ChannelSftp c_tFTPFileList_1 = (com.jcraft.jsch.ChannelSftp) globalMap
						.get("conn_tFTPConnection_1");

				if (c_tFTPFileList_1 != null && c_tFTPFileList_1.getSession() != null) {
					log.info("tFTPFileList_1 - Use an existing connection. Connection username: "
							+ c_tFTPFileList_1.getSession().getUserName() + ", Connection hostname: "
							+ c_tFTPFileList_1.getSession().getHost() + ", Connection port: "
							+ c_tFTPFileList_1.getSession().getPort() + ".");
				}
				...
				String remotedir_tFTPFileList_1 = "/SUNSHINE_CONCUR/EXTRACT_INPUT_FILES/".replaceAll("\\\\", "/");
				...
				log.info("tFTPFileList_1 - Listing files from server.");
	...
	public void tFTPGet_1Process(final java.util.Map<String, Object> globalMap) throws TalendException {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
					...
					class BytesLimit65535_tFTPGet_1 {
						public void limitLog4jByte() throws Exception {
							...
							log4jParamters_tFTPGet_1
									.append("LOCALDIR" + " = " + "\"/Users/8kmilesinc./Desktop/TalendInput/Concur\"");
							log4jParamters_tFTPGet_1.append(" | ");
							log4jParamters_tFTPGet_1
									.append("REMOTEDIR" + " = " + "\"/SUNSHINE_CONCUR/EXTRACT_INPUT_FILES/\"");
							log4jParamters_tFTPGet_1.append(" | ");
							...
							log4jParamters_tFTPGet_1.append(" | ");
							log4jParamters_tFTPGet_1.append("PRINT_MESSAGE" + " = " + "true");
							log4jParamters_tFTPGet_1.append(" | ");
							if (log.isDebugEnabled())
				...
				String remotedir_tFTPGet_1 = "/SUNSHINE_CONCUR/EXTRACT_INPUT_FILES/";
				...
				String localdir_tFTPGet_1 = "/Users/8kmilesinc./Desktop/TalendInput/Concur";
//create folder if local direcotry (assigned by property) not exists
	...
	public static void main(String[] args) {
		final SFTP_CONN SFTP_CONNClass = new SFTP_CONN();

		int exitCode = SFTP_CONNClass.runJobInTOS(args);
		if (exitCode == 0) {
			log.info("TalendJob: 'SFTP_CONN' - Done.");
		}
	...
	public int runJobInTOS(String[] args) {
		...
		log.info("TalendJob: 'SFTP_CONN' - Start.");
		...
		try {
			...
			java.io.InputStream inContext = SFTP_CONN.class.getClassLoader()
					.getResourceAsStream("uscomm_sunshine_rpt_qa/sftp_conn_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
				inContext = SFTP_CONN.class.getClassLoader()
		...
		if (false) {
			System.out.println((endUsedMemory - startUsedMemory) + " bytes memory increase when running : SFTP_CONN");
		}
		if (enableLogStash) {
...
/************************************************************************************************
 * 75118 characters generated by Talend Cloud Data Management Platform on the
 * March 9, 2022 at 11:34:22 AM CST
 ************************************************************************************************/